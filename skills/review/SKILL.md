...
- Reviewer markers are colored rounded-square initials. Done comments remain in inbox history.
...
- Do not mark comments Done for the user. After the revised preview is verified, record implemented comment IDs in the
  private receipt so they are not reprocessed; the user marks them Done after visual approval.
