...
test('syncs versioned assets once and then reports a no-op', async t => {
  ...
  assert.equal(manifest.packageVersion, '0.2.10');
  ...
  await writeFile(resolve(publicDirectory, 'review-prototype.js'), '// altered after sync\n');
  const restored = await execute(process.execPath, [cli, 'sync', '--public-dir', publicDirectory, '--format', 'json']);
  assert.equal(JSON.parse(restored.stdout).changed, true);
  assert.match(await readFile(resolve(publicDirectory, 'review-prototype.js'), 'utf8'), /ReviewPrototype/);
